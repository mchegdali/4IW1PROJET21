...
const {
  ...
  defaultValues: livraisonData
});

const [fullname, fullnameField] = defineField('fullname');
const [emailCustomer, emailCustomerField] = defineField('emailCustomer');
...
const submitHandler = handleSubmit(async (data, signal) => {
  const response = await fetch(`${config.apiBaseUrl}/shipping`, {
    method: 'POST',
    headers: {
        ...
        <div id="shipping" class="space-y-5">
          ...
          <div class="flex flex-col">
            ...
            <Input
              ...
              type="text"
            />
            <small v-if="formErrors.street">{{ formErrors.street }}</small>
          </div>
          <div class="flex flex-col">
            ...
            <fieldset>
              ...
              >
                ...
                <label
                  ...
                  class="w-full py-4 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                  >Livraison en point de retrait Mondial Relay (choix du point relais après
                  paiement)</label
                >
              </div>
            </fieldset>
          ...
          <div class="flex flex-col gap-5">
            ...
            <fieldset>
              ...
              <div class="items-center p-4 border border-gray-200 rounded dark:border-gray-700">
                <Input
                  @click="displayCredForm"
                  id="bordered-radio-3"
                  type="radio"
                  value=""
                  name="bordered-radio-payment"
                  class="inline-flex w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                />
                <label
                  for="bordered-radio-3"
                  class="w-full py-4 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                  >Carte de Credit</label
                >
              ...
              <label> PayPal </label>
              <div
                @click="displayPayPalForm"
                class="items-center ps-4 border border-gray-200 rounded dark:border-gray-700"
              >
                <Input
                  checked
                  id="bordered-radio-4"
                ...
                <div id="paypalForm" v-bind:style="dispPayPal">
                  <p>
                    Après avoir cliqué sur "Payer avec PayPal" Vous serez redirigé(e) vers PayPal
                    pour finaliser votre achat de façon sécurisée
                  </p>
                </div>
              </div>
            </fieldset>
          </div>
          <div v-if="displayPaypalBtn" class="#">
            <Button type="submit" class="w-1/2 bg-blue-900" :disabled="isSubmitting"
              >Payer avec PayPal</Button
            >
          </div>
          <div v-else>
            <Button type="submit" class="w-1/2" :disabled="isSubmitting">Payer Maintenant</Button>
          </div>
        </div>
      </form>
    </div>
