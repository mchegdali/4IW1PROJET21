...
const livraisonQuerySchema = z.object({
  page: z.coerce.number().int().min(1).default(1),
  text: z.string().min(2).optional(),
});

const livraisonCreateSchema = z
  .object({
...
const livraisonUpdateSchema = livraisonCreateSchema.innerType().partial();

const livraisonSchema = livraisonCreateSchema
  .innerType()
  .merge(slugSchema)
  .merge(entitySchema)
