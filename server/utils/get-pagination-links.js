function getPaginationLinks(reqUrl, page, text, pageSize, metadata) {
  const lastPage = Math.ceil(metadata?.[0].total / pageSize);
  const selfUrl = new URL(reqUrl, process.env.HOST);
  ...
  const lastUrl = new URL(reqUrl, process.env.HOST);
  ...
  let prevUrl = page - 1 === 0 ? null : new URL(reqUrl, process.env.HOST);
  ...
  let nextUrl = page + 1 >= lastPage ? null : new URL(reqUrl, process.env.HOST);
  if (nextUrl) {
